...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 YOUR CODE 提示:如何将处理器fd中的文件加载到进程内存中？如何设置argc/argv？
     * 宏或函数:
     *  mm_create        - 创建一个内存管理器（mm）
     *  setup_pgdir      - 在mm中设置页目录（pgdir）
     *  load_icode_read  - 读取程序文件的原始数据内容
     *  mm_map           - 构建新的虚拟内存区域（vma）
     *  pgdir_alloc_page - 为TEXT/DATA/BSS/堆栈部分分配新的内存
     *  lcr3             - 更新页目录地址寄存器（CR3）
     */
  // 你可以按照已完成的LAB5中的代码来完成以下步骤
 /* (1) 为当前进程创建一个新的内存管理器
     * (2) 创建一个新的页目录表（PDT），并将mm->pgdir设置为PDT的内核虚拟地址
     * (3) 将二进制文件的TEXT/DATA/BSS部分复制到进程的内存空间中
     *    (3.1) 读取文件中的原始数据内容并解析elfhdr
     *    (3.2) 根据elfhdr中的信息，读取文件中的原始数据内容并解析proghdr
     *    (3.3) 调用mm_map构建与TEXT/DATA相关的vma
     *    (3.4) 调用pgdir_alloc_page为TEXT/DATA分配页，读取文件中的内容并将其复制到新分配的页中
     *          (3.5) 调用pgdir_alloc_page为BSS分配页，在这些页中使用memset将其置零
     * (4) 调用mm_map设置用户堆栈，并将参数放入用户堆栈中
     * (5) 设置当前进程的内存管理器、cr3，重置pgidr（使用lcr3宏）
